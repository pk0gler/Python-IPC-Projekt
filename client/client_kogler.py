...
class Node(object):
    """
    Node Class
    Holds all important values for one node
    including its fieldtype and its weight
    """
    def __init__(self, x, y, val):
        """init Method

        constructs a Node Object with supplied values
    ...
    def __lt__(self, other):
        """lt Method for ordering in Priority Queu
...
def create_commands(path, start):
    """Create commands for path

    used with a star in combination
...
def calc_possible_new_fields(val, coor, map):
    """Calculation of the amount of near fields

    takes the coordianates of a field
    and calculates its neighbors iwch arent known and of course reachable
...
def draw_tile(graph, id, style, width):
    """draw tiles for debugging and visualization
...
class SquareGrid:
    """
    Sqauregrid for creating a sqaure graph
    """
...
class PriorityQueue:
    """
    Priority Queue for prioritizing the path
    """
    def __init__(self):
        """
        construct a proorityqueue object
        """
        self.elements = []
...
def main(argv):
    ...
    timeout = 0.5
    if len(argv) != 8:
        raise ValueError("Invalid Arguments -> >>client-kogler -h localhost -p 5050 -x 10 -y 10<<")
    # Set port globally
    try:
        ip = argv[1]
        port = int(argv[3])
